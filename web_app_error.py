...
try:
    ...
    print("✅ CHẾ ĐỘ NÂNG CAO ĐÃ KÍCH HOẠT (web_backtest): Mô phỏng đầy đủ BE + TS from Version3")
    ADVANCED_MODE = True
    ...
    try:
        ...
        print("✅ CHẾ ĐỘ NÂNG CAO ĐÃ KÍCH HOẠT: Mô phỏng đầy đủ BE + TS from top-level Version3")
        ADVANCED_MODE = True
    except Exception as e_top:
        _advanced_import_error = e_top
        print(f"⚠️ Không thể tải module nâng cao (package and top-level failed): {e_top}")
    ...
    try:
        ...
        if candle_path:
            if os.path.exists(candle_path):
                df_candle = load_candle_csv_from_content(open(candle_path, 'r', encoding='utf-8').read())
        else:
            # Try common candidates in workspace and tradelists
            candidates = [f"{symbol}", f"{symbol}.csv", f"{symbol.replace(' ', '')}.csv", f"{symbol.upper()}.csv"]
            found = False
            for sp in ['.', os.path.join('.', 'tradelists')]:
                for c in candidates:
            ...
            if df_candle is None:
                # Last resort: search files containing symbol
...
# ============================================================================
# 🎯 PRIORITY 1: MULTI-SYMBOL BATCH PROCESSING ROUTES
# ============================================================================

@app.route('/test_batch')
def test_batch():
    return "<h1>Test Batch Works!</h1>"
...
def batch_dashboard():
    ...
    try:
        ...
        return """
        <html>
        <head><title>Batch Processing</title></head>
        <body>
            <h1>🚀 Batch Processing Dashboard</h1>
            <p>Multi-symbol optimization coming soon!</p>
...
def comparison_dashboard():
    ...
    try:
        return """
        <html>
        <head><title>Compare Results</title></head>
        <body>
            <h1>🔍 Compare Results</h1>
            <p>Result comparison tools coming soon!</p>
            <a href="/">Back to Home</a>
        </body>
        </html>
        """
...
if __name__ == '__main__':
    ...
    print("🎯 New Features: Multi-Symbol Batch Processing & Comparison Dashboard")
...
def batch_optimize():
    ...
    try:
        ...
        if not config.get('symbols') or not config.get('timeframes'):
            return jsonify({'success': False, 'error': 'Must select at least one symbol and timeframe'})
        
        # Generate unique batch ID
        batch_id = f"batch_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        # Store batch configuration (in a real implementation, use proper session/database storage)
        batch_configs[batch_id] = {
            'config': config,
            'status': 'starting',
            'progress': 0,
            'symbol_progress': [],
        ...
        # Start batch processing in background (simplified implementation)
        # In production, use Celery or similar for proper background processing
        import threading
        thread = threading.Thread(target=process_batch, args=(batch_id, config))
        thread.daemon = True
        thread.start()
...
# Global storage for batch configs (in production, use Redis/Database)
batch_configs = {}

def process_batch(batch_id, config):
    """Process batch optimization in background"""
    try:
        batch_configs[batch_id]['status'] = 'running'
        ...
        for symbol in config['symbols']:
            for timeframe in config['timeframes']:
                try:
                    # Update progress
                    batch_configs[batch_id]['current_symbol'] = symbol
                    batch_configs[batch_id]['current_timeframe'] = timeframe
                    batch_configs[batch_id]['progress'] = int((completed / total_combinations) * 100)
                    
                    # Load data
                    candle_data = data_manager.load_candle_data(symbol, timeframe)
                    if candle_data is None or candle_data.empty:
                        print(f"⚠️ No candle data for {symbol} {timeframe}m, skipping...")
                        completed += 1
                        continue
                    
                    # Find tradelist file for this symbol
                    tradelist_files = data_manager._discover_tradelist_files(symbol)
                    if not tradelist_files:
                        print(f"⚠️ No tradelist for {symbol}, skipping...")
                        completed += 1
                        continue
                    
                    # Load and process tradelist
                    tradelist_path = tradelist_files[0]  # Use first available
                    trade_data = tradelist_manager.load_tradelist(tradelist_path)
                    if trade_data is None or trade_data.empty:
                        print(f"⚠️ No trade data for {symbol}, skipping...")
                        completed += 1
                        continue
                    
                    # Get trade pairs
                    trade_pairs, trade_log = tradelist_manager.get_trade_pairs(trade_data)
                    if not trade_pairs:
                        print(f"⚠️ No valid trade pairs for {symbol}, skipping...")
                        completed += 1
                        continue
                    
                    # Run optimization
                    if config['opt_method'] == 'optuna':
                        # Use Optuna optimization
                        best_result = run_optuna_optimization(
                            trade_pairs, candle_data, 
                            config['sl_min'], config['sl_max'],
                            config['be_min'], config['be_max'],
                            config['ts_min'], config['ts_max'],
                            n_trials=100  # Reduced for batch processing
                        )
                    else:
                        # Use grid search
                        sl_list = list(np.arange(config['sl_min'], config['sl_max'] + config['sl_step'], config['sl_step']))
                        be_list = list(np.arange(config['be_min'], config['be_max'] + config['be_step'], config['be_step']))
                        ts_list = list(np.arange(config['ts_min'], config['ts_max'] + config['ts_step'], config['ts_step']))
                        
                        best_result = run_grid_optimization(trade_pairs, candle_data, sl_list, be_list, ts_list, ts_list)
                    
                    if best_result:
                        # Store result
                        result_id = results_manager.store_result(
                            symbol=symbol,
                            timeframe=str(timeframe),
                            strategy="SL_BE_TS_V3",
                            parameters=best_result.get('params', {}),
                            metrics=best_result,
                            metadata={'batch_id': batch_id, 'opt_method': config['opt_method']}
                        )
                        ...
                        results.append({
                            ...
                            'timeframe': timeframe,
                            'result_id': result_id,
                            'best_pnl': best_result.get('pnl', 0),
                            'best_winrate': best_result.get('winrate', 0),
                            'best_pf': best_result.get('pf', 0)
                        })
                        
                        print(f"✅ Completed {symbol} {timeframe}m: PnL={best_result.get('pnl', 0):.2f}")
                    
                    completed += 1
                    
                except Exception as e:
                    print(f"❌ Error processing {symbol} {timeframe}m: {e}")
                    completed += 1
                    continue
        
        # Mark batch as completed
        batch_configs[batch_id]['status'] = 'completed'
        batch_configs[batch_id]['progress'] = 100
        batch_configs[batch_id]['results'] = results
        batch_configs[batch_id]['end_time'] = datetime.now()
        
        print(f"🎯 Batch {batch_id} completed: {len(results)} successful optimizations")
        
    except Exception as e:
        batch_configs[batch_id]['status'] = 'error'
        batch_configs[batch_id]['error'] = str(e)
...
def batch_status(batch_id):
    ...
    if batch_id not in batch_configs:
        return jsonify({'error': 'Batch not found'}), 404
    
    batch = batch_configs[batch_id]
    ...
    if batch['status'] == 'running':
        message = f"Processing {batch.get('current_symbol', '')} {batch.get('current_timeframe', '')}m... ({elapsed_str})"
    elif batch['status'] == 'completed':
        message = f"Completed {len(batch.get('results', []))} optimizations in {elapsed_str}"
    elif batch['status'] == 'error':
        message = f"Error: {batch.get('error', 'Unknown error')}"
    else:
    ...
    return jsonify({
        ...
        'symbol_progress': batch.get('symbol_progress', [])
    })

def run_optuna_optimization(trade_pairs, candle_data, sl_min, sl_max, be_min, be_max, ts_min, ts_max, n_trials=100):
    """Run Optuna optimization for batch processing"""
    try:
        def objective(trial):
            sl = trial.suggest_float('sl', sl_min, sl_max)
            be = trial.suggest_float('be', be_min, be_max)
            ts_trig = trial.suggest_float('ts_trig', ts_min, ts_max)
            ts_step = trial.suggest_float('ts_step', 0.1, 1.0)
            ...
            return performance.get('pnl', -1000)
...
def run_grid_optimization(trade_pairs, candle_data, sl_list, be_list, ts_trig_list, ts_step_list):
    """Run grid search optimization for batch processing"""
    try:
        ...
        # Find best result
        best_result = max(results, key=lambda x: x.get('pnl', -1000))
        return best_result
...
def _generate_symbol_checkboxes(symbols):
    """Generate HTML for symbol selection checkboxes"""
    html = ""
    for symbol in symbols:
        html += f'''
        <div class="symbol-card" id="symbol_{symbol}" data-symbol="{symbol}" onclick="toggleSymbol('{symbol}')">
            <strong>{symbol}</strong>
        </div>
        '''
    return html

def _generate_timeframe_checkboxes(timeframes):
    """Generate HTML for timeframe selection checkboxes"""
    html = ""
    for tf in timeframes:
        html += f'''
        <label style="margin-right: 15px;">
            <input type="checkbox" name="timeframes" value="{tf}"> {tf}m
        </label>
        '''
    return html

# Add helper methods to app class (monkey patching for simplicity)
app._generate_symbol_checkboxes = _generate_symbol_checkboxes
app._generate_timeframe_checkboxes = _generate_timeframe_checkboxes
